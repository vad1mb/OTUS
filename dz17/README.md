...
Данная ошибка прерывает выполнение скриптов из раздела provision. Однозначного решения этой проблемы не найдено.  Для предотвращения этой ошибки отключаем проброс директории в Vagrantfile:
